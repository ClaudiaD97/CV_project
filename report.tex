...
\subsubsection*{Implementation choices}
We chose to use Alexnet, a convolutional neural network trained on the ImageNet dataset (1.2 million high-resolution images belonging to 1000 categories) proposed by Krizhevsky in 2012 \cite{4}.
...
We evaluated the network on the test images and we got a test accuracy of about 0.86. In order to improve the test accuracy of our classifier we tried to perform data augmentation as described in section 2.1. We still obtain a test accuracy around 87\%, so we don't get any significant improvement. In Figure \ref{fig:alexnetresults} the confusion matrices for both cases are reported but they are almost identical.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.49\textwidth]{img/conf_finetune}
	\includegraphics[width=0.49\textwidth]{img/alexnet_augmented.png}
	\caption{Confusion matrix of the finetuned Alexnet without and with data augmentation}	
...
\begin{figure}[h!]
	...
	\caption{Confusion matrix of the multiclass linear SVM classifier}	
	\label{fig:svm}
\end{figure}
...
\begin{description}
	...
	\begin{figure}[h!]
		...
		\caption{Confusion matrices using polynomial kernel of degree 2 and 3}
		\label{fig:poly23}
	\end{figure}
